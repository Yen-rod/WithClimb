document.addEventListener('DOMContentLoaded', function() {
    ...
    var table = $('#viasTable').DataTable({
            ...
            { data: 'totalAscensos' },
            {
                ...
                render: function(data) {
                    ...
                    <div class="flex gap-2">
                        <a href="/admin/routes/view/${data}" class="text-blue-600 hover:text-blue-800">
                              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                <path d="M10 12a2 2 0 100-4 2 2 0 000 4z" />
                                <path fill-rule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clip-rule="evenodd" />
                             </svg>
                        </a>
                        <a href="/admin/routes/update/${data}" class="text-yellow-600 hover:text-yellow-800">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                            </svg>
                        </a>
                        <button onclick="showDeleteModal(${data})" data-id="${data}" class="delete-via-btn text-red-600 hover:text-red-800">
                              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
        ...
        responsive: true,
        columnDefs: [{ targets: [1, 2, 3, 4, 5], className: 'desktop' }],
        language: {
    ...
    async function deleteVia(id, button) {
        try {
            ...
            if (result.status === 'success') {
                table.ajax.reload();
                notificationSystem.show('Vía eliminada correctamente', 'success');
                hideModal();
            } else {
                notificationSystem.show(result.message || 'Error al eliminar la vía', 'error');
    ...
    deleteModal.addEventListener('click', e => { if (e.target === this) hideModal(); });
    ...
    $(document).on('click', () => $('.mobile-menu').removeClass('active'));
    $(document).on('click', '.mobile-menu', e => e.stopPropagation());
});